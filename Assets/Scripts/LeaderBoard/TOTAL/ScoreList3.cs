...
public class ScoreList3 : MonoBehaviour
{
    ...
    public void ShowLeaderboard (string boardName, string boardTitle, string valueName)
    {
        ...
        var leaderboards = PlayfabManager3.GetLeaderboard(boardName);

		if (leaderboards.Count > 0)
		{
			if (boardName == "tot_time")
			{
				foreach (var leaderboard in leaderboards)
				{
					...
					entry.transform.Find("Value").GetComponent<TextMeshProUGUI>().text = clock;
				}
			}
			else if (boardName == "deaths")
			{
