...
public class TriWeapon : MonoBehaviourPun
{
    #region Variables
            ...
            foreach (Touch touch in Input.touches)
            {
                if ((touch.position.x > Screen.width * 0.33f) && (touch.position.y > Screen.height * 0.2f))
                {
                    ...
                    if (touch.phase == TouchPhase.Ended)
                    {
                        ...
                        {
                            photonView.RPC("shootBRay", RpcTarget.All);
    ...
    void shootBRay()
    {
        shoot_ray = cam.ViewportPointToRay(new Vector3(0.5f,0.5f,0));
        ...
        if (Physics.Raycast(shoot_ray, out hit, range, can_shoot))
        {
            ...
            GameObject laser2 = Instantiate(bullet2, muzzle2.position, muzzle3.rotation);
            GameObject laser3 = Instantiate(bullet2, muzzle2.position, muzzle3.rotation);
            laser.GetComponent<OnlineShotBehavior>().setTarget(hit.point);
            laser2.GetComponent<OnlineShotBehavior>().setTarget(hit.point);
            laser3.GetComponent<OnlineShotBehavior>().setTarget(hit.point);
            GameObject.Destroy(laser, 2f);
            GameObject.Destroy(laser2, 2f);
            GameObject.Destroy(laser3, 2f);
            if (photonView.IsMine)
            {
        ...
        else if (Physics.Raycast(shoot_ray, out hit, range))
        {
            laser_sound.Play();
            GameObject laser = Instantiate(bullet, muzzle.position, muzzle.rotation);
            GameObject laser2 = Instantiate(bullet, muzzle2.position, muzzle3.rotation);
            GameObject laser3 = Instantiate(bullet, muzzle2.position, muzzle3.rotation);
            laser.GetComponent<OnlineShotBehavior>().setTarget(hit.point);
            laser2.GetComponent<OnlineShotBehavior>().setTarget(hit.point);
            laser3.GetComponent<OnlineShotBehavior>().setTarget(hit.point);
            GameObject.Destroy(laser, 2f);
